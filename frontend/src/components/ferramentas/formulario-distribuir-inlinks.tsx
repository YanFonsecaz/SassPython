...
const STEPS = [
  { label: "URL alvo", icon: TargetIcon },
  { label: "Candidatas", icon: LinkIcon },
  { label: "Confirmar", icon: SparklesIcon },
] as const;

export function FormularioDistribuirInlinks() {
  ...
  useEffect(() => {
    async function loadCusto() {
      try {
        const dados = await api.get<CustoDistribuirInlinksResponse>(
          `/ferramentas/distribuir-inlinks/custo?n_candidatas=${candidatasUrls.length}`
        );
  ...
  const canAdvance = useCallback(() => {
    switch (step) {
      case 0: return !!urlAlvo.trim();
      case 1: return candidatasUrls.length >= 1;
      case 2: return true;
      default: return false;
    }
  }, [step, urlAlvo, candidatasUrls]);
  ...
  function addUrlsEmLote() {
    ...
    const novas = linhas.filter((u) => u !== alvoNorm && !candidatasUrls.includes(u));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const resultado = await api.post<ExecucaoCriada>(
        "/ferramentas/distribuir-inlinks",
        body
      );
        ...
        {step === 0 && (
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="url-alvo"
                placeholder="https://seublog.com/pagina-alvo"
                maxLength={2048}
                value={urlAlvo}
          ...
          <div className="space-y-5 animate-fade-in">
            ...
            {!modoLote ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    id="nova-url"
                    placeholder="https://seublog.com/artigo-candidato"
                    maxLength={2048}
                    value={novaUrl}
            ...
            <div className="grid gap-4 sm:grid-cols-3 pt-2">
              <div className="space-y-2">
                ...
                <Input
                  id="max-inlinks-candidata"
                  type="number"
                  min={1}
                  max={3}
                  value={maxInlinksPorCandidata}
                  onChange={(e) => setMaxInlinksPorCandidata(Number(e.target.value))}
                  disabled={enviando}
                />
              </div>
              <div className="space-y-2">
